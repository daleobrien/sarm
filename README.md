...
sarm only runs on apple silicon (arm64).
...
*sarm* serves static error pages when a client's request results in an error, eg 404. The pages live in `www/err/(code).html` and are embedded at build time.
See [Configuration](#configuration) to modify the default file and docroot.

## Running
- `./sarm` to start running the web server on `127.0.0.1:8080`.
- `./sarm [port]` to start running the web server on `127.0.0.1:[port]`
- `./sarm [any-letter]` is accepted as a legacy "debug" flag, but has no effect anymore — sarm no longer forks, so there is nothing to disable. (Kept so old invocations don't break.)
...
To see sarm in action, start running sarm with `./sarm [port]`. Then open your web browser of choice (or use curl), and visit `127.0.0.1:8080/` or `127.0.0.1:8080/pretty/index.html`. Bask in the warmth of assembly.

## What can it do?
sarm is a static-file web server — every file under `www/` is embedded into the binary at build time, so content is served with no filesystem access and no heap allocation. It speaks both HTTP/1.1 and HTTP/2 (RFC 9113).
...
## "Safety"
This is a web server written entirely by-hand in ARM64 assembly as a fun project. It's probably got a lot of vulnerabilities I'm unaware of. However, I did do my best to make it safer. Here are some safety precautions sarm takes.
...
Earlier versions of sarm supported CGI scripts, PUT uploads, DELETE, and directory listing. Those features have been **removed** — sarm is a read-only static-file server again (everything embedded at build time). The old implementations can still be found in the git history.

## HTTP Status Codes
sarm's current handlers can reply with the following status codes:
...
## Configuration
You can configure sarm with the `config.S` file. The options are documented here.
- `#define DOCROOT "www/"` -- This is the docroot, prepended to every requested path (and the root of the embedded table). Change it to wherever your HTML files are, relative to sarm, or use an absolute path:
  - `#define DOCROOT "www/"`
  - `#define DOCROOT "/Library/WebServer/Documents`
  - `#define DOCROOT "./"`
- `#define ERR_DIR "www/err/"` -- This is the directory in which sarm looks for custom error HTML pages in the embedded table, eg, `www/err/404.html` or `www/err/500.html`
- `#define DEFAULT_FILE "index.html"` -- This is the default file sarm will serve when it receives an empty `GET / HTTP/1.1` request
- `.equ RESPONSE_HEADER_SIZE, 512` -- Maximum bytes the response header may occupy; a larger header is an error (500)
- `.equ RECV_TIMEOUT, 10` -- Number of seconds sarm will wait to receive data before closing the connection. If it's more than `RECV_TIMEOUT` seconds between `read()`s, sarm will close the connection with `408 Request Timed Out`
...
sarm is written for macOS and Linux; platform differences are handled in-tree behind `#ifdef __linux__` in `defs.S`, `main.S` and `child.S`:
...
- `SO_NOSIGPIPE` doesn't exist on Linux. sarm instead ignores `SIGPIPE` and handles the `EPIPE` from `write()`.
...
### Special Thanks:
- [ymawky](https://github.com/imtomt/ymawky), an ARM HTTP server, was a big inspiration
