...
// Stage 13: connection management — PING (6.7): an 8-octet PING without
// ACK is answered with a PING ACK carrying the identical payload, wrong
// lengths and non-zero stream ids are rejected, and an ACK is never
// answered; GOAWAY (6.8): the server records a client GOAWAY, answers
// with its own GOAWAY(NO_ERROR), and refuses any brand-new stream with
...
// ═══════════════════════════════════════════════════════════════════
//  Stage 13 — connection management (RFC 9113 §6.7, §6.8)
// ═══════════════════════════════════════════════════════════════════

// ── 13.1 — PING at the dispatch level ──────────────────────────────
// RFC 9113 §6.7: a PING carries exactly 8 opaque octets and lives on
// stream 0; a plain PING is answered with a PING ACK carrying the
// identical payload, and a PING that already has the ACK flag is never
// answered.
...
// ── 13.2 — GOAWAY at the dispatch level ────────────────────────────
// RFC 9113 §6.8: stream id 0 and at least 8 payload octets; the peer's
// last stream id is recorded, the connection is marked going away, and
// the server answers with its own GOAWAY(NO_ERROR) carrying its
// high-water mark. A HEADERS that would open a brand-new stream
// afterwards is refused with REFUSED_STREAM; streams already in the
// table (trailers) are untouched.
...
// ── 13.1 — PING over a real connection ─────────────────────────────
// The client sends a PING with a distinctive 8-octet payload; the
// connection loop must answer with a PING ACK carrying the identical
// payload (RFC 9113 §6.7).
...
// ── 13.2 — GOAWAY over a real connection ───────────────────────────
// The client sends GOAWAY(NO_ERROR) and then — violating the protocol —
// a HEADERS for a brand-new stream. The server must answer GOAWAY with
// its own GOAWAY(NO_ERROR), refuse the new stream with
// RST_STREAM(REFUSED_STREAM), and never serve it.
...
// GET /big.bin (70000 bytes, larger than the 65535-byte initial window)
// through the full connection loop. The client lets the credit run dry
// — the server is mid-response, blocked in h2_write_body — then sends
// GOAWAY. The server must dispatch it (answering with its own GOAWAY)
// without killing the in-flight stream, resume on WINDOW_UPDATE, and
// deliver the whole body: an accepted stream finishes before the
// connection terminates (§6.8).
...
int main(void) {
	...
	// Stage 13 — connection management
