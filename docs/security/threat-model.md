...
|---|---|---|
| ECDSA P-256 private scalar (32 B) | `src/tls/cert_data.S` (`tls_priv_key`), generated by `certs/embed_cert.sh` from `certs/key.pem` | the life of the binary — static, read-only since Step 13 |
| Certificate DER | `src/tls/cert_data.S:13` (`tls_cert_der`) | public; copied to the wire verbatim, never parsed |

The private scalar is embedded as literal `.byte` data. Since Step 13 that
data is **read-only** — `__DATA_CONST` on macOS, `.rodata` on Linux, mapped
`r--` in the running process and checked by name in
`tests/hardening_checks.py` — so it can be read by anything with an
in-process read primitive, and rewritten by nothing. `certs/key.pem` is still
checked into the repository. Today's key is a self-signed `localhost`
development certificate, so nothing confidential is currently exposed — but
the shape is exactly the one `docs/SECURITY.md` §9 warns about, and choosing
between its four options is a deployment decision, costed out in
[hardening.md](hardening.md) §7. Recorded in §9.
...
**Changed in Step 13.** Everything above *was* emitted into writable
`.data`/`.bss`, with nothing in a read-only section — the state observation 2
in §9 recorded. It no longer is: `src/defs.S` has a `rodata` macro
(`__DATA_CONST,__const` on Mach-O, `.rodata` on ELF) and everything the server
never writes now uses it. The buffers in the table above are still writable,
because they are written; the embedded assets, the certificate, the **private
scalar**, the crypto constants and every dispatch table are not. That is 160 KB
on macOS and 154 KB on Linux moved out of writable memory, checked against the
linked binary and against the running process by `tests/test_hardening.sh`. See
...
| `setrlimit` (macOS) / `prlimit64` (Linux) | `main.S:513,520` | startup — `RLIMIT_CORE` to zero, before the listening socket exists (Step 13) |
...
1. ~~**The private scalar is embedded in writable `.data`**~~, and
   `certs/key.pem` is in the repository. **Half closed in Step 13**: the
   scalar is no longer writable — it is in `__DATA_CONST`/`.rodata`, checked by
   name in the linked binary and mapped read-only in the running process — so a
   write primitive can no longer replace the key in place. It is still
   *readable* in process memory, as any in-process key is, which is what Step
   10's leak probe covers. The deployment story (§9 of `docs/SECURITY.md`:
   per-deployment or hardware-backed key) is still open, and is now analysed
   rather than merely deferred: [hardening.md](hardening.md) §7 costs out all
   four options against this build. → partly fixed; the remainder is a
   deployment decision.
2. ~~**No section is read-only.**~~ **Closed in Step 13.** Embedded assets,
   the certificate, the Huffman, MIME, status and HPACK static tables, the
   crypto constants, the frame dispatch table and the private scalar are all in
   a read-only section now (`rodata` in `src/defs.S`), and the four tables that
   held pointers hold link-time offsets instead — so the binary needs no
   load-time relocations, which is also what let the Linux build become a
   position-independent executable with a non-executable stack. §5 has the
   sizes; [hardening.md](hardening.md) has the argument, and
   `tests/test_hardening.sh` has the check, with a `-DSARM_NO_RODATA` control
   build to show it can fail. → fixed.
...
10. **The exported `ghash` symbol has no caller in the server, and the GCM
   ...
   **not** addressed by Step 13, which turned out to be the wrong home for it:
   Step 13 is binary and OS protections, and this is a code-duplication finding
   in the GCM tree. It belongs with whatever step next touches
   `src/crypto/gcm/` under the `verified-asm-crypto` workflow. → still open.
