...
- **Build**: `Makefile` compiles every `src/**/*.S` (recursively — one file per function, grouped in module folders) into `.o` via `cc -g -c`, then links with `ld -l System -e _main -arch arm64`.
...
| Path | Role |
|---|---|
| `src/config.S` | All user-configurable constants (`#define` + `.equ`): docroot, timeouts, limits |
| `src/defs.S` | Syscall numbers, struct offsets, macros (`adr_l`, `cb`), constants, shared length constants; `#include`s `config.S` |
| `src/data.S` | Global `.data`/`.bss` shared by all modules: `buf`, `clientfd`, `file_des`, `request`, `response`, embedded table pointers, `connection_mode`, `h2_preface` |
| `src/embedded.S` | Generated by `embed_www.sh` — the `www/` assets as binary data (never edited by hand) |
...
| `src/parse/*.S` | HTTP/1 + HTTP/2 request parsing: `get_header_field`, `parse_range`, `parse_h2_path`, `parse_path`, `parse_header_end`, `parse_request` (+ shared docroot/filename buffers) |
...
| `src/http1/*.S` | HTTP/1 response encoding + method handlers: `find_http_code` (status table), `http1_write_response`, `reply_status` (error pages), `handle_fs_error`, `create_response`, `get_setup`, `get`, `head`, `options` |
| `src/h2/*.S` | HTTP/2 frame engine (RFC 9113): frame parse/validate/dispatch, all frame handlers, stream engine, `h2_build_request`, `h2_connection_loop`, response encoders |
...
## GET / HEAD (`src/http1/get.S`, `src/http1/head.S`)
...
## Response System (`src/http1/http1_write_response.S`)
...
## File System Operations (`src/file/`)
