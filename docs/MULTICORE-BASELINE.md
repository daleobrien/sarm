...
- ClientHello delivered fragmented across ten single-byte writes (a real
  ...
  unfragmented write. (A ClientHello generated by Python's `ssl` module —
  distinct from curl/LibreSSL's — fails the handshake with or without
  fragmentation, identically on the pre-Step-9 commit; that's a
  pre-existing gap in what this from-scratch TLS 1.3 implementation
  accepts, unrelated to this step.)
